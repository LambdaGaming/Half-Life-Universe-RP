--Custom persistence system that only saves basic entity info
...
hook.Add( "PlayerSay", "playersaypersist", function( ply, text, public )
	...
	if entnum[1] == "!save" and ply:IsSuperAdmin() then
		...
		if !entnum[2] then
			ply:ChatPrint( "Please add an identifier as an argument." )
			return
		end
		if !IsValid( tr ) or tr:CreatedByMap() or tr:IsWorld() or tr:IsPlayer() then
			ply:ChatPrint( "This entity cannot be saved." )
			return
		end
