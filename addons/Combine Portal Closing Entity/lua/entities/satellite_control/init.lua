...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create( "satellite_control" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Use(caller, activator)
	...
	if caller.hascodes != true then DarkRP.notify( caller, 1, 5, "You don't have the codes to shutdown the portal!" ) return end
	if GetConVar("blowout_enabled"):GetInt() == 1 then DarkRP.notify( caller, 1, 6, "ERROR: The portal is already being opened!" ) return end
	...
	for k, ply in pairs( player.GetAll() ) do
		DarkRP.notify( ply, 0, 8, "Codes sent to Combine overworld.......... 2 minutes until portal detonation." )
	end
end