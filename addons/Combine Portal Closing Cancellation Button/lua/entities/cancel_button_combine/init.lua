...
function ENT:Use( caller, activator )
	...
	if GetConVar("blowout_enabled"):GetInt() == 0 then DarkRP.notify( caller, 1, 6, "ERROR: Nothing was detected to cancel." ) return end
	RunConsoleCommand( "blowout_enabled", 0 )
	timer.Remove( "blowoutcombine" )
	timer.Remove( "changelevelcombine" )
	for k, ply in pairs(player.GetAll() ) do
		DarkRP.notify( ply, 1, 8, "Portal failed to successfully close, emergency cancellation button was activated." )
	end
end